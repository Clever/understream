...
describe '_.batch(n)', ->
  it 'waits for n items, then outputs them in an array', (done) ->
    _(_.range 20).stream().batch(10).value (data) ->
      assert.deepEqual data, [_.range(10), _.range(10, 20)]
      done()
    .run assert.ifError

  it 'outputs all items if n is greater than the number of items in the stream', (done) ->
    _(_.range 5).stream().batch(6).value (data) ->
      assert.deepEqual data, [_.range(5)]
      done()
    .run assert.ifError

  it 'outputs any items it has if the input stream ends', (done) ->
    _(_.range 13).stream().batch(10).value (data) ->
      assert.deepEqual data, [_.range(10), _.range(10, 13)]
      done()
    .run assert.ifError
